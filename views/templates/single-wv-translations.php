                    ...
                    <article id="post-<?php the_ID(); ?>" <?php post_class( $has_transliteration ); ?>>
                        <div class="translation-item">                   
                            <div class="content">
                                <h1><?php the_title(); ?></h1>
                                <div class="meta">
                                    <span class="singer"><strong><?php _e( 'Singer', PLUGIN_KEY ); ?>:</strong>
                                        <?php foreach( $singers as $singer ): ?>
                                            <a href="<?php echo esc_url( get_term_link( $singer ) ) ?>"><?php echo esc_html( $singer->name ); ?></a>
                                        <?php endforeach; ?>
                                    </span>
                                    <span class="author"><strong><?php _e( 'Author', PLUGIN_KEY ); ?>: </strong>
                                        <?php the_author_posts_link(); ?>
                                    </span>
                                    <span class="the-date"><strong><?php _e( 'Published on', PLUGIN_KEY ); ?>: </strong>
                                        <?php the_date(); ?>
                                    </span>                            
                                </div>
                                <div class="content">
